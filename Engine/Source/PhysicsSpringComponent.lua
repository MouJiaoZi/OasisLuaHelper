...
---@field SpringStiffness number @Specifies how much strength the spring has. The higher the SpringStiffness the more force the spring can push on a body with.
---@field SpringDamping number @Specifies how quickly the spring can absorb energy of a body. The higher the damping the less oscillation
---@field SpringLengthAtRest number @Determines how long the spring will be along the X-axis at rest. The spring will apply 0 force on a body when it's at rest.
---@field SpringRadius number @Determines the radius of the spring.
---@field SpringChannel ECollisionChannel @Strength of thrust force applied to the base object.
---@field bIgnoreSelf boolean @If true, the spring will ignore all components in its own actor
---@field SpringCompression number @The current compression of the spring. A spring at rest will have SpringCompression 0.
