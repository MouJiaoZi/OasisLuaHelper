...
---@field TextStyle FTextBlockStyle @The button style used at runtime
---@field SelectedKey FInputChord @The currently selected key chord.
---@field Font_DEPRECATED FSlateFontInfo
---@field Margin FMargin @The amount of blank space around the text used to display the currently selected key.
---@field ColorAndOpacity_DEPRECATED FLinearColor
---@field KeySelectionText string @Sets the text which is displayed while selecting keys.
---@field NoKeySpecifiedText string @Sets the text to display when no key text is available or not selecting a key.
---@field bAllowModifierKeys boolean @input chord representing the selected key, if false modifier keys are ignored.
---@field bAllowGamepadKeys boolean @When true gamepad keys are allowed in the input chord representing the selected key, otherwise they are ignored.
---@field EscapeKeys ULuaArrayHelper<FKey> @When true gamepad keys are allowed in the input chord representing the selected key, otherwise they are ignored.
---@field OnKeySelected FOnKeySelected @Called whenever a new key is selected by the user.
---@field OnIsSelectingKeyChanged FOnIsSelectingKeyChanged @Called whenever the key selection mode starts or stops.
